...
<div class="account-page">
    ...
    <aside class="sidebar { isMobileView ? 'mobile' : '' } { isMenuOpen ? 'open' : '' }">
        ...
        <nav class="side-nav">
            ...
            {#if $currentUser?.role === UserRole.Programmer || $currentUser?.role === UserRole.Moderator || $currentUser?.role === UserRole.Administrator}
                <button
            ...
            {#if $currentUser?.role === UserRole.Administrator || $currentUser?.role === UserRole.Moderator}
                ...
                <button
                    class={ activeTab === Tab.STATUSES ? 'active' : '' }
                    on:click={ () => setTab(Tab.STATUSES) }
                >
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6H4"/><path d="M20 12H4"/><path d="M20 18H4"/></svg>
                    Статусы
                </button>
    ...
    <div class="main-content {isMobileView ? 'mobile' : ''}" transition:fade={{ duration: 200 }}>
        ...
        {:else}
            ...
            {:else if activeTab === Tab.USERS}
                <Users />
            {:else if activeTab === Tab.STATUSES}
                <Statuses />

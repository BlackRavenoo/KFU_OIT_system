<script lang="ts">
    ...
    /**
     * Извлекает идентификатор страницы из URL. 
     * Если идентификатор найден, возвращает его в виде числа, иначе возвращает null.
     * @throws {Error} Если URL не может быть обработан, функция возвращает null без выбрасывания исключения.
     * @returns {number | null} Идентификатор страницы или null, если идентификатор не найден или произошла ошибка при обработке URL.
     */
    ...
    /**
     * Осуществляет навигацию назад в истории браузера.
     */
    function goBack() { history.back(); }

    /**
     * Загружает аватар по данным автора и отображает его внутри указанного элемента. 
     * Если загрузка аватара не удалась, отображает первую букву имени автора или '?' в случае отсутствия имени.
     * @param {HTMLDivElement} node - HTML элемент, в котором будет отображаться аватар
     * @param {Author | undefined} author - Данные автора, включая id, имя и ключ аватара
     * @returns {Object} Объект с методом update для обновления аватара при изменении данных автора
     */
    ...
    /**
     * Загружает данные страницы и её контент. Устанавливает состояние загрузки и обрабатывает возможные ошибки при загрузке данных страницы и её контента.
     * @throws {Error} Если загрузка страницы или её контента не удалась, устанавливает соответствующее сообщение об ошибке в состояние `error` или `contentError`.
     * @returns {Promise<void>} Промис, который разрешается после завершения загрузки страницы и её контента.
    */
    ...
    /**
     * Инициирует процесс удаления страницы, отображая окно подтверждения удаления. 
     */
    function requestDelete() { showDeleteConfirm = true; }
    /**
     * Отменяет процесс удаления страницы, скрывая окно подтверждения удаления.
     */
    function cancelDelete() { showDeleteConfirm = false; }

    /**
     * Подтверждает удаление страницы. Выполняет запрос к серверу на удаление страницы по её идентификатору.
     * @throws {Error} Если удаление страницы не удалось, отображает уведомление с сообщением об ошибке.
     * @returns {Promise<void>} Промис, который разрешается после завершения процесса удаления страницы.
     */
    ...
    /**
     * Добавляет обработчики событий для элементов кода и цитирования, позволяя пользователю копировать их содержимое в буфер обмена.
     */
    ...
    /**
     * При монтировании компонента устанавливает заголовок страницы и описание, а также выполняет начальную загрузку страницы.
    */
    onMount(() => {

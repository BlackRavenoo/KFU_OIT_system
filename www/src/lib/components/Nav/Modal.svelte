<script lang="ts">
    ...
    export let captchaComponent: any;
    ...
    */
     ...
     * @emits login - Событие, отправляемое при успешной валидации формы логина. Содержит данные: { login, password, remember, captchaToken }
     * @emits update - Событие, отправляемое при возникновении ошибки валидации капчи. Содержит данные: { loginError }
     * @throws {Error} Если возникает ошибка при валидации капчи или отправке формы
    */
     ...
     * @emits reset - Событие, отправляемое при успешной валидации формы сброса пароля. Содержит данные: { email, captchaToken }
     * @emits update - Событие, отправляемое при возникновении ошибки валидации капчи. Содержит данные: { loginError }
     * @throws {Error} Если возникает ошибка при валидации капчи или отправке формы
    */
    ...
    /**
     * Добавляет глобальный обработчик нажатия клавиш при монтировании компонента и удаляет его при уничтожении.
    */
    onMount(() => {
    ...
    /**
     * Удаляет глобальный обработчик нажатия клавиш при уничтожении компонента, чтобы предотвратить утечки памяти 
     * и нежелательное поведение при взаимодействии с другими частями приложения.
    */
    onDestroy(() => {
            ...
            <div class="modal-content">
                ...
                <div class="warning-message">
                    ...
                    <div class="warning-text">
                        <span>Внимание!</span>
                        <p>Вход в систему доступен только для сотрудников КФУ</p>
                    </div>
                </div>
                ...
                {#if !isReseting}
                    <div class="login-container">
                        ...
                        <button class="submit-btn" on:click={ handleLoginSubmit }>
                ...
                {:else}
                    <div class="reset-container">
                        ...
                        <button class="submit-btn" on:click={ handleResetSubmit }>

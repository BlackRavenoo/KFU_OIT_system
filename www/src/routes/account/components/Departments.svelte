<script lang="ts">
    ...
    /**
     * Начало редактирования отдела: установка текущего редактируемого отдела и заполнение поля для редактирования его данными.
     * @param {Department} d - отдел, который нужно редактировать
     */
    ...
    /**
     * Отмена редактирования: сброс текущего редактируемого отдела и очистка поля для редактирования.
     * При этом изменения, внесенные в поле редактирования, не сохраняются.
     */
    function cancelEdit() {
        editingId = null;
        editName = '';
    ...
    /**
     * Сохранение изменений при редактировании отдела: отправка запроса на обновление отдела с новым названием, 
     * обновление списка отделов в интерфейсе и отображение уведомления об успешном обновлении.
      - Если редактируемый отдел не выбран (editingId == null) или новое название пустое, 
      функция просто возвращает без выполнения дальнейших действий.
      - В случае успешного обновления отдела, список отделов обновляется с помощью метода update, 
      который заменяет старый отдел на новый с обновленным названием.
     * @returns {Promise<void>} - промис, который разрешается после завершения операции сохранения изменений. 
        В случае ошибки промис будет отклонен с соответствующим сообщением об ошибке.
     */
    ...
    /**
     * Добавление нового отдела: отправка запроса на создание отдела с указанным названием, 
     * обновление списка отделов в интерфейсе и отображение уведомления об успешном создании.
      - Если новое название пустое, функция просто возвращает без выполнения дальнейших действий.
      - В случае успешного создания отдела, поле для ввода названия очищается, 
      а список отделов обновляется с помощью метода update, который добавляет новый отдел в начало списка.
     * @returns {Promise<void>} - промис, который разрешается после завершения операции добавления нового отдела. 
        В случае ошибки промис будет отклонен с соответствующим сообщением об ошибке.
     */
    ...
    /**
     * Открытие модального окна для подтверждения удаления отдела: установка текущего удаляемого отдела и отображение модального окна.
     * @param {Department} d - отдел, который нужно удалить
     */
    ...
    /**
     * Закрытие модального окна для подтверждения удаления отдела: сброс текущего удаляемого отдела и скрытие модального окна.
     */
    ...
    /**
     * Подтверждение удаления отдела: отправка запроса на деактивацию отдела, \
     * обновление списка отделов в интерфейсе и отображение уведомления об успешной деактивации.
     * @returns {Promise<void>} - промис, который разрешается после завершения операции подтверждения удаления отдела. 
        В случае ошибки промис будет отклонен с соответствующим сообщением об ошибке.
     */
    ...
    /**
     * Проверка прав доступа при загрузке компонента: если текущий пользователь не является администратором, 
     * перенаправление на страницу с ошибкой 403.
    */
    onMount(() => {

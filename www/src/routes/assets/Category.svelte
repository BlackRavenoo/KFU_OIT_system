<script lang="ts">
    ...
    /**
     * Обрабатывает сохранение новой категории. Проверяет валидность формы, отправляет данные на сервер и обрабатывает ответ.
    ...
    /**
     * Обработчик нажатия клавиш. Закрывает модальное окно при нажатии клавиши Escape.
    ...
    /**
     * Устанавливает слушатель событий для нажатия клавиш при монтировании компонента и удаляет его при уничтожении, 
     * обеспечивая возможность закрытия модального окна с помощью клавиши Escape и предотвращая утечки памяти от неиспользуемых слушателей.
    */
    onMount(() => {
    ...
    /**
     * Удаляет слушатель событий для нажатия клавиш при уничтожении компонента, предотвращая утечки памяти и обеспечивая корректное управление ресурсами при закрытии модального окна.
    */
    onDestroy(() => {
